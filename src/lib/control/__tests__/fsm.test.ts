...
describe('ActionFSM', () => {
  ...
  describe('State Utilities', () => {
    ...
    it('should return valid target states', () => {
      ...
      expect(ActionFSM.getValidTargets('running')).toEqual([
        'succeeded',
        'failed',
      ]);
      ...
      expect(transitions).toEqual([
        {
          from: 'queued',
          to: 'running',
          description: 'Action started execution',
        },
        {
          from: 'running',
          to: 'succeeded',
  ...
  describe('Sequence Validation', () => {
    it('should validate valid sequences', () => {
      expect(
        ActionFSM.validateSequence(['queued', 'running', 'succeeded'])
      ).toBe(true);
      expect(ActionFSM.validateSequence(['queued', 'running', 'failed'])).toBe(
        true
      );
    ...
    it('should reject invalid sequences', () => {
      ...
      expect(ActionFSM.validateSequence(['queued', 'running', 'queued'])).toBe(
        false
      );
    });
  ...
  describe('Transition Descriptions', () => {
    it('should return correct descriptions for valid transitions', () => {
      expect(ActionFSM.getTransitionDescription('queued', 'running')).toBe(
        'Action started execution'
      );
      expect(ActionFSM.getTransitionDescription('running', 'succeeded')).toBe(
        'Action completed successfully'
      );
      expect(ActionFSM.getTransitionDescription('running', 'failed')).toBe(
    ...
    it('should return null for invalid transitions', () => {
      expect(
        ActionFSM.getTransitionDescription('queued', 'succeeded')
      ).toBeNull();
      expect(
        ActionFSM.getTransitionDescription('succeeded', 'running')
      ).toBeNull();
      expect(
        ActionFSM.getTransitionDescription('invalid', 'running')
      ).toBeNull();
    });
  });
});
...
describe('ActionStateUtils', () => {
  ...
  describe('Next States', () => {
    it('should return next possible states', () => {
      ...
      expect(ActionStateUtils.getNextStates('running')).toEqual([
        'succeeded',
        'failed',
      ]);
...
describe('Idempotent Transitions', () => {
  it('should reject idempotent transitions (running -> running)', () => {
    expect(ActionFSM.isValidTransition('running', 'running')).toBe(false);
    expect(() => ActionFSM.guard('running', 'running')).toThrow(
      "Invalid state transition: running -> running. Valid transitions from 'running': succeeded, failed"
    );
  });

  it('should reject idempotent transitions (queued -> queued)', () => {
    expect(ActionFSM.isValidTransition('queued', 'queued')).toBe(false);
    expect(() => ActionFSM.guard('queued', 'queued')).toThrow(
      "Invalid state transition: queued -> queued. Valid transitions from 'queued': running"
    );
  });

  it('should reject idempotent transitions (succeeded -> succeeded)', () => {
    expect(ActionFSM.isValidTransition('succeeded', 'succeeded')).toBe(false);
    expect(() => ActionFSM.guard('succeeded', 'succeeded')).toThrow(
      "Invalid state transition: succeeded -> succeeded. Valid transitions from 'succeeded': "
    );
  });

  it('should reject idempotent transitions (failed -> failed)', () => {
    expect(ActionFSM.isValidTransition('failed', 'failed')).toBe(false);
    expect(() => ActionFSM.guard('failed', 'failed')).toThrow(
      "Invalid state transition: failed -> failed. Valid transitions from 'failed': "
    );
  });
});
    ...
    try {
      ...
      if (error instanceof Error) {
        expect(error.message).toContain(
          'Invalid state transition: queued -> succeeded'
        );
        expect(error.message).toContain(
          "Valid transitions from 'queued': running"
        );
      }
    }
  });

  it('should throw standard Error objects (not ApiError) for FSM validation', () => {
    try {
      ActionFSM.guard('running', 'queued');
      fail('Should have thrown an error');
    } catch (error) {
      // FSM should throw standard Error objects, not ApiError
      // This keeps the FSM pure and HTTP-agnostic
      expect(error).toBeInstanceOf(Error);
