  ...
  describe('GET /api/status', () => {
    ...
    it('returns cached status when within 30 second window', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/status?cardId=test-card-1'
      );
    ...
    it('returns 404 when card is not found', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/status?cardId=nonexistent'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('handles validation errors', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/status?cardId=invalid'
      );
      const response = await GET(request);
      const data = await response.json();
