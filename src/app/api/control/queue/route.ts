...
export const GET = withAgentAuth(
  async (request: NextRequest, principal: Principal) => {
    try {
        ...
        return await prisma.$transaction(async (tx) => {
          ...
          for (const action of queuedActions) {
            try {
              ...
              console.error(
                `Invalid state transition for action ${action.id}: ${action.status} -> running`
              );
              throw new Error(
                `Cannot lock action ${action.id}: ${fsmError instanceof Error ? fsmError.message : 'Invalid state transition'}`
              );
            }
          }
      ...
      if (actions.length === 0) {
        const response = ResponseHelper.noContent('agent');
        response.headers.set('Connection', 'keep-alive');
        return response;
      }

      // Return actions as JSON with keep-alive connection
      const response = ResponseHelper.success(actions, 'agent');
      response.headers.set('Connection', 'keep-alive');
      return response;
    } catch (error) {
      ...
      if (
        ...
        error.message.includes('Cannot lock action')
      ) {
        return createErrorResponse(
