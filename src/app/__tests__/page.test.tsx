...
jest.mock('@/components/lab-card', () => ({
  LabCard: ({ id, title, description, group }: { id: string; title: string; description: string; group?: string }) => (
    <div data-testid="lab-card" data-title={title} data-group={group || 'Development'}>
      <h3>{title}</h3>
      <p>{description}</p>
...
describe('HomePage', () => {
  ...
  describe('Rendering', () => {
    it('renders the main portal header', () => {
      ...
      expect(screen.getByText('LAB TOOLS')).toBeInTheDocument()
  ...
  describe('LabCardsGrid', () => {
    ...
    it('fetches cards on mount', async () => {
      ;(global.fetch as jest.Mock).mockResolvedValueOnce(mockFetchResponse(mockCards))
      
      render(<HomePage />)
      
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith('/api/cards', { cache: 'no-store' })
      })
    })
    ...
    it('handles fetch error gracefully', async () => {
      ;(global.fetch as jest.Mock).mockResolvedValueOnce(mockFetchError('Failed to fetch', 500))
      
      render(<HomePage />)
      
      await waitFor(() => {
        expect(screen.getByText('No Lab Tools Available')).toBeInTheDocument()
        expect(screen.getByText('There are currently no lab tools configured. Please check back later or contact an administrator.')).toBeInTheDocument()
      })
    })
    ...
    it('shows no results message when search has no matches', async () => {
      ...
      expect(screen.getByText(/No lab tools match your search for "nonexistent"/)).toBeInTheDocument()
    })
  })
  ...
  describe('Category Grouping', () => {
    ...
    it('sorts cards within categories by order', () => {
      const developmentCards = screen.getAllByTestId('lab-card').filter(card => 
        card.getAttribute('data-group') === 'Development'
      )
      
      expect(developmentCards[0]).toHaveAttribute('data-title', 'Test Lab Tool 1')
      expect(developmentCards[1]).toHaveAttribute('data-title', 'Test Lab Tool 2')
    })
  })
  ...
  describe('Navigation', () => {
    ...
    it('provides admin access link in no tools message', async () => {
      ...
      await waitFor(() => {
        ...
        expect(adminAccessButton.closest('a')).toHaveAttribute('href', '/admin/login')
      })
    })
  })
  ...
  describe('Responsive Design', () => {
    it('applies responsive grid classes', () => {
      ...
      const gridContainer = screen.getByText('LAB TOOLS').closest('div')?.parentElement?.parentElement?.querySelector('.grid')
      expect(gridContainer).toHaveClass('grid', 'gap-8', 'md:grid-cols-2', 'lg:grid-cols-3')
    })
  })
