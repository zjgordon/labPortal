...
export class Agent {
  ...
  private async executeAction(action: any): Promise<void> {
    ...
    try {
      // Report that we're starting work immediately (as per requirements)
      await this.portalClient.reportActionStatus(id, 'running');
      this.logger.info(`Action ${id} status reported as running`);
      ...
      if (result.isTimeout) {
        // Handle timeout case specifically (as per requirements)
        ...
        this.logger.warn(`Action ${id} timed out after ${result.duration}ms`);
      } else {
